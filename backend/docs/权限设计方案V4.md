...
```sql
-- 分配给超级管理员角色
INSERT INTO role_resource (role_id, resource_id, permission_type)
SELECT 1, id, 'basic' FROM resource WHERE module_code = 'system';

-- 分配给 admin 用户（user_id=1）
INSERT INTO user_resource (user_id, resource_id, permission_type)
VALUES (1, 101, 'direct'), (1, 102, 'direct'), (1, 103, 'direct'), (1, 104, 'direct');
```
...
| API | `API` | 接口权限 | PAGE/ELEMENT | 获取用户列表 API |
| PERMISSION | `PERMISSION` | 独立权限 | MODULE | 后台任务权限 |
...
| 表名 | 用途 | 关键字段 |
|------|------|----------|
| `resource` | 资源表 | id, name, code, type, parent_id, permission_key, module_code |
| `permission` | 权限表 | id, permission_key, name, resource_id |
| `role` | 角色表 | id, role_name, role_code, type |
| `user` | 用户表 | id, username, real_name, company_id |
| `role_resource` | 角色 - 资源关联 | role_id, resource_id, permission_type |
| `user_resource` | 用户 - 资源关联 | user_id, resource_id, permission_type |
...
CREATE TABLE resource (
    ...
    type                VARCHAR(20) NOT NULL,         -- MODULE/MENU/PAGE/ELEMENT/API/PERMISSION
    parent_id           BIGINT REFERENCES resource(id), -- 父资源 ID
    permission_key      VARCHAR(200) UNIQUE,          -- 权限标识
