...
| **V4.2** | **2026-04-04** | **资源与权限分离设计 + 审计日志 + 软删除** | **柳生/AI** |
...
INSERT INTO role_resource (role_id, resource_id, is_basic, is_effective)
...
│  - 关联关系（role_resource）：权限的体现（谁拥有什么）          │
...
**注**：已取消 `PERMISSION` 资源类型，避免混淆。权限通过 `role_resource` 表管理。
...
| `role_resource` | 角色 - 资源关联表 | role_id, resource_id, is_basic, is_default, is_effective | **权限关系**：角色拥有哪些资源 |
| `user_resource_adjust` | 用户资源调整表 | user_id, resource_id, adjust_type (ADD/REMOVE) | 用户权限调整记录 |
...
CREATE TABLE role_resource (
    ...
    is_basic            BOOLEAN DEFAULT false,                     -- 是否基本权限（必须生效，不可移除）
...
CREATE INDEX idx_role_resource_effective ON role_resource(role_id, is_effective);
CREATE INDEX idx_role_resource_resource ON role_resource(resource_id);
```

### 5.4 user_resource_adjust 表结构（用户权限调整）

```sql
CREATE TABLE user_resource_adjust (
    id                  BIGSERIAL PRIMARY KEY,
...
用户最终权限 = 
    (从所有角色继承的有效权限并集)
    + (user_resource_adjust 中 ADD 的资源)
    - (user_resource_adjust 中 REMOVE 的资源，但排除角色基本权限)
```

**索引设计**：
```sql
CREATE INDEX idx_user_adjust_user ON user_resource_adjust(user_id);
CREATE INDEX idx_user_adjust_type ON user_resource_adjust(user_id, adjust_type);
```
...
**优先级顺序**：
1. 用户直接权限（user_resource_adjust） > 角色权限
...
用户最终权限 = 
    (所有角色的有效权限并集)
    - (user_resource_adjust 中 REMOVE 的权限，排除角色基本权限)
    + (user_resource_adjust 中 ADD 的权限)
```
...
**工作内容**：
- [ ] 创建 `resource` 表（从旧表迁移）
- [ ] 创建 `role_resource` 表（从旧 role_permission 迁移）
- [ ] 创建 `user_resource_adjust` 表（从旧 user_permission_adjust 迁移）
...
-- 4. 从旧表迁移数据
INSERT INTO role_resource (role_id, resource_id, is_basic, is_default, is_effective, ...)
...
| **权限** | 角色/用户对资源的访问权（通过 role_resource 表体现） |
...
| **权限体现** | 混淆在表名中 | role_resource 表 | 权限关系明确 |
